...
test_that("make_func_call_explicit passes code with actual test file content", {
  test_file <- system.file("R", "test_file.R", package = "rpkgkit")
  test_code_str <- paste(readLines(test_file, warn = FALSE), collapse = "\n")
  ...
  local_mocked_bindings(
    readLines = function(path, ...) test_code_str,
    .package = "base"
  )
  ...
  expect_equal(add_double_colons_calls[[1L]]$code, test_code_str)
  expect_equal(writeLines_calls[[1L]]$text, test_code_str)
})
