test_that("check_pkgdown_reference defaults pkg to '.' when NULL", {
  ...
  local_mocked_bindings(
    readLines = function(f, ...) c("export(myfunc)", "export(otherfunc)"),
    .package = "base"
  )

  local_mocked_bindings(
    read_yaml = function(file, ...) {
      list(reference = list(contents = c("myfunc", "otherfunc")))
    },
    .package = "yaml"
  )

  suppressMessages(
    result <- check_pkgdown_reference()
  )
  expect_equal(result, character())
})

test_that("check_pkgdown_reference defaults pkg to '.' when empty string", {
  tmp <- tempfile("pkg_test")
  dir.create(tmp)
  on.exit(unlink(tmp, recursive = TRUE))
  owd <- setwd(tmp)
  ...
  local_mocked_bindings(
    readLines = function(f, ...) character(),
    .package = "base"
  )

  local_mocked_bindings(
    read_yaml = function(file, ...) {
      list(reference = list(contents = character()))
    },
    .package = "yaml"
  )
